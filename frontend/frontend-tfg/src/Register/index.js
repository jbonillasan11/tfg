...
const Register = () => {
    ...
    function registerInfoSend() {
        ...
        if ([email, password, name, surname, org].some(field => !field?.trim())) { //Si falta algún dato, no enviamos nada
            setUnfilledFields(true);
            return;
        }
        ...
        if (password != passwordRepeat) { //Si las pwd no coinciden, no enviamos nada
            setPwdMismatch(true);
            return;
        }
